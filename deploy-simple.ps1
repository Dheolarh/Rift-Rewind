...
# --- Add Lambda permissions for API Gateway endpoints ---
Write-Host "Adding Lambda permissions for API Gateway..." -ForegroundColor Yellow

# You may need to update these values if your API Gateway ID or account changes
$apiId = aws apigateway get-rest-apis --region $Region --query "items[?name=='RiftRewindAPI'].id" --output text
$accountId = aws sts get-caller-identity --query 'Account' --output text

# /analyze POST
$analyzeSourceArn = "arn:aws:execute-api:${Region}:${accountId}:${apiId}/*/POST/analyze"
aws lambda add-permission --function-name RiftRewindOrchestrator --statement-id apigateway-access-analyze-$(Get-Random) --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn $analyzeSourceArn --region $Region 2>$null | Out-Null

# /api/regions GET
$regionsSourceArn = "arn:aws:execute-api:${Region}:${accountId}:${apiId}/*/GET/api/regions"
aws lambda add-permission --function-name RiftRewindOrchestrator --statement-id apigateway-access-regions-$(Get-Random) --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn $regionsSourceArn --region $Region 2>$null | Out-Null

Write-Host "OK Lambda permissions added for API Gateway endpoints" -ForegroundColor Green
